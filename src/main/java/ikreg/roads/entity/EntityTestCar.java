...
public class EntityTestCar extends Entity
{
    ...
    {
        if (entityIn instanceof EntityTestCar)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityTestCar.Status.UNDER_WATER && this.status != EntityTestCar.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityTestCar.Status getCarStatus()
    {
    	EntityTestCar.Status entityboat$status = this.getUnderwaterStatus();
        ...
        {
            return EntityTestCar.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityTestCar.Status.ON_LAND;
            }
            else
            {
                return EntityTestCar.Status.IN_AIR;
            }
        }
    }
     ...
     */
    private EntityTestCar.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.water && d0 < (double)func_184452_b(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                            	EntityTestCar.Status entityboat$status = EntityTestCar.Status.UNDER_FLOWING_WATER;
                                return entityboat$status;
                            }
        ...
        return flag ? EntityTestCar.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityTestCar.Status.IN_AIR && this.status != EntityTestCar.Status.IN_AIR && this.status != EntityTestCar.Status.ON_LAND)
        {
            ...
            this.status = EntityTestCar.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityTestCar.Status.IN_WATER)
            {
                d2 = (this.field_184465_aD - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityTestCar.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityTestCar.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityTestCar.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityTestCar.Status.ON_LAND)
            {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityTestCar.Status.ON_LAND)
                    {
    ...
    public EntityTestCar.Type getCarType()
    {
    ...
    public static enum Type
    {
        ...
        public static EntityTestCar.Type getTypeFromString(String nameIn)
        {
